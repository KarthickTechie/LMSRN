...
const MasterPage = () => {
  ...
  const [mastersNotDownloaded,setMasterNotDownloaded] = useState<Master[]>(masterData)
...
'>
     { mastersDownloaded.length > 0 && mastersDownloaded.map((item,index)=>{
      return (
        <MasterCard 
        key={index} 
      ...
      { mastersNotDownloaded.length > 0 && mastersNotDownloaded.map((item,index)=>{
      ...
      }
      {
        ...
        <FontAwesome  name="thumbs-up" size={40} color="#FF9C01"
        className='m-10'
        />
